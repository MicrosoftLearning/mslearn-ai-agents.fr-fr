...
1. Sous le commentaire **Initialiser l'outil MCP de l'agent**, Ajoutez le code suivant :
   ...
   mcp_tool = McpTool(
    ...
    Ce code se connectera à un serveur MCP git distant qui permet aux agents IA de se connecter et d'aider à répondre aux questions relatives à la documentation d'un dépôt github existant. 
...
1. Localisez le commentaire **Créer et traiter l'exécution de l'agent dans un thread à l'aide des outils MCP** et ajoutez le code suivant :
   ...
   print(f"Created run, ID: {run.id}")
    ...
    L'agent IA invoquera automatiquement les outils MCP connectés pour traiter la demande immédiate. Pour illustrer ce processus, le code fourni sous le commentaire **Afficher les étapes d'exécution et les appels d'outils** affichera tous les outils invoqués à partir du serveur MCP.
...
1. Une fois la connexion effectuée, entrez la commande suivante pour exécuter l’application :
   ...
   python client.py
    ...
    Notez que l'agent a pu invoquer automatiquement l'outil MCP `fetch_azure_rest_api_docs` pour répondre à la demande.
